...
const rectValues = {
  ...
  x: '328.33',
  y: '-2.22',
  transform: 'translate(159.71, 300.23) rotate(-60.92)',
};
    ...
    set({
      ...
      onRest: () => {
        if (shouldTriggerSpin) {
          reportAnalytics(analyticsActions.click, 'pull to spin');
          props.onClickSpin();
        }
        setZeroRest();
      },
  ...
  const bind = useGesture({
    ...
    onDragEnd: ({ movement }) => {
      if (movement[0] < 1 && movement[1] > -1) {
        ...
        reportAnalytics(analyticsActions.drag, 'pull to spin');
        props.onClickSpin();
      }
    },
  });
    ...
    <g className="PullToSpin">
      ...
      <g role="button" className="PullSlider">
        <animated.g {...bind()} transform={animation ? transform : dragAnim} className="PullAnimatedGroup">
          ...
          />
          <rect className="" rx="2" ry="2" fill="white" opacity="1" {...rectValues} />
          <rect className="PullSpinRect" rx="2" ry="2" fill="url(#diagonalHatch)" {...rectValues} />
        </animated.g>
      </g>
    </g>
  );
