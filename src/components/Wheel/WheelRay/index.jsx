          ...
          <React.Fragment key={`${item.id}CompanyRay`}>
            {
              (!this.state.flagLayouts) ? null : (
                <g
                  ...
                  {...handleAnalyticsInteraction('condition_wheel', 'feature', item.name, props.onChangeDot, index)}
                >
                  <CompanyFlag
                    y={-65}
            ...
            also to can be used to mark the search */}
            {
              (item.name.charAt(0) === legendTracker) ? null : (
                <text
                  ...
                  {...handleAnalyticsInteraction('condition_wheel', 'feature', item.name, props.onChangeRay, index)}
                >
                  {item.name.charAt(0)}
                </text>
              )
        ...
        : (
          <React.Fragment key={`${item.id}LocationRay`}>
            <g
              ...
              {...handleAnalyticsInteraction('condition_selected_region', 'feature', item.name, props.onChangeRay, index)}
            >
              <LocationRay
                regionId={item.id}
                ...
                ? (
                  <g key="text" transform={`${transform} translate(28.75)`} className="locationRay">
                    ...
                    <text
                      ...
                      transform="rotate(94)"
                      textAnchor="right"
                      {...handleAnalyticsInteraction('condition_selected_region', 'feature', item.province, props.onChangeRay, index)}
                    >
                      &nbsp;{item.province}
                      <title>
                        {item.province}
